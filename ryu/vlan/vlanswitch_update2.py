...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    #字典mac_to_port and trunk_to_port初始化
    ...
    #分類經過trunk_port的src是否有帶vlan，如果沒有則在封包添加vlan_tag=0，如果有則再添加一次vlan_tag
    ...
    #設定其src所對應的vlan_tag，爲了讓封包在經過trunk_port到另一臺交換機時可重新添加vlan
    ...
    #分類in_port 第一種爲一般port 第二種爲trunk_port 第三種爲vlan_port
    def breakdown_in_port(self,in_port,src,dpid,pkt,eth,dst):
     ...
     #如果爲有帶vlan則發往相同vlan的port
    def vlan_to_vlan(self,dst,dpid,src_vlan,parser,ofproto,msg,datapath,in_port,src):
    	actions = []
    ...
    #設定經trunk_port的out_port 如果爲normal_port則發往normal_port
	...
	if src_vlan !='NULL' and src_vlan !=0:
		self.vlan_to_vlan(dst,dpid,src_vlan,parser,ofproto,msg,datapath,in_port,src)
	elif src_vlan == 0:
